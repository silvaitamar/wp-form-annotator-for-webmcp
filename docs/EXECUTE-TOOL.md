...
Keys must match the HTML `name` of each control (the same keys shown in **Settings → WebMCP Forms**). IDs below are examples from a clean Studio install — copy names from the rendered page if they differ.
...
| SureForms | `submit_contact` | `execute-tool-sureforms.json` | Shortcode + `render_block` SSR |

## How to run

1. Enable the form in **Settings → WebMCP Forms** and fill `toolparamdescription` for each field.
...
- **SureForms:** dropdowns use Tom Select + a hidden input. The adapter strips `aria-hidden` and hidden `name` so the native `<select>` is what WebMCP sees. Copy real `name` values from the page into the fixture.
